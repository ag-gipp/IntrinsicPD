...
class FeatureExtractor:
    ...
    def yule_k_characteristic(self, sent_spans_index_start, sent_spans_index_end):
        ...
        try:
            return 10000/self.yule_i(sent_spans_index_start, sent_spans_index_end)
        except:
            return 0

    #adapted from https://github.com/jpotts18/stylometry/blob/master/stylometry/extract.py (seenu.andi-rajendran)
    def type_token_ratio(self, sent_spans_index_start, sent_spans_index_end):
        spans = self.sentence_spans[sent_spans_index_start:sent_spans_index_end]
        start, end = spans[0][0], spans[-1][1]
        text = self.text[start:end]
        wds = nltk.tokenize.word_tokenize(text)
        uniq_wds = set(wds)
        return float(len(uniq_wds))/float(len(wds))

    #adapted from https://github.com/ikoniaris/PythonExercises/blob/master/ex36.py seenu.andi-rajendran
    def hapax_legomena(self, sent_spans_index_start, sent_spans_index_end):
        spans = self.sentence_spans[sent_spans_index_start:sent_spans_index_end]
        start, end = spans[0][0], spans[-1][1]
        text = self.text[start:end]
        wds = nltk.tokenize.word_tokenize(text)
        z=Counter(wds)
        once_seen_wds = set([wd for wd in wds if z[wd] == 1])
        return float(len(once_seen_wds)) / float(len(wds))

    # average word length function definition seenu.andi-rajendran
    def average_word_length(self, sent_spans_index_start, sent_spans_index_end):
        spans = self.sentence_spans[sent_spans_index_start:sent_spans_index_end]
        start, end = spans[0][0], spans[-1][1]
        text = self.text[start:end]
        wds = nltk.tokenize.word_tokenize(text)
        return float(sum([len(wd) for wd in wds])/len(wds))

    #adapted from http://stackoverflow.com/questions/17531684/n-grams-in-python-four-five-six-grams (seenu.andi-rajendran)
    def _char_ngram_ratio(self,sent_spans_index_start, sent_spans_index_end, n,inp):
        spans = self.sentence_spans[sent_spans_index_start:sent_spans_index_end]
        start, end = spans[0][0], spans[-1][1]
        text = self.text[start:end]
        cnt=0
    ...
    #adapted from https://gist.github.com/magnusnissel/d9521cb78b9ae0b2c7d6 (seenu.andi-rajendran)
    def yule_i(self, sent_spans_index_start, sent_spans_index_end):
        m1 = 0.0
        m2 = 0.0
        for counts in self.__wordcount(sent_spans_index_start, sent_spans_index_end).values():
            m1 += counts
            m2 += counts ** 2
        try:
            return (m1*m1)/(m2-m1)
    ...
    #adapted from https://github.com/ikoniaris/PythonExercises/blob/master/ex36.py seenu.andi-rajendran
    def hapax_dislegomena(self, sent_spans_index_start, sent_spans_index_end):
        spans = self.sentence_spans[sent_spans_index_start:sent_spans_index_end]
        start, end = spans[0][0], spans[-1][1]
        text = self.text[start:end]
        wds = nltk.tokenize.word_tokenize(text)
        z=Counter(wds)
        twice_seen_wds = set([wd for wd in wds if z[wd] == 2])
        return float(len(twice_seen_wds))/float(len(wds))

    def __wordcount(self, sent_spans_index_start, sent_spans_index_end):
        spans = self.sentence_spans[sent_spans_index_start:sent_spans_index_end]
        start, end = spans[0][0], spans[-1][1]
        text = self.text[start:end]
        wds = nltk.tokenize.word_tokenize(text)
    ...
    #counting all special characters(seenu.andi-rajendran)
    def spec_char_count(self, sent_spans_index_start, sent_spans_index_end):
        spans = self.sentence_spans[sent_spans_index_start:sent_spans_index_end]
        start, end = spans[0][0], spans[-1][1]
        text = self.text[start:end]
        _count = 0
        for each_char in text:
            if each_char in "~`!@#$%^&*()_-+={}[]:>;',</?*-+":
                _count += 1
        return float(_count / len(text))


    def char_gram_ratio(self, sent_spans_index_start, sent_spans_index_end):
        spans = self.sentence_spans[sent_spans_index_start:sent_spans_index_end]
        start, end = spans[0][0], spans[-1][1]
        text = self.text[start:end]
        n = 4
        ngs = [text[i:i + n] for i in range(len(text) - n + 1)]
        c=Counter(ngs)
        return float(c.most_common(1)[0][1])/float(len(ngs))

    #counting alphabets (seenu.andi-rajendran)
    def alpha_chars_ratio(self, sent_spans_index_start, sent_spans_index_end):
        spans = self.sentence_spans[sent_spans_index_start:sent_spans_index_end]
        start, end = spans[0][0], spans[-1][1]
        text = self.text[start:end]
        alpha_count=0
        for each_char in text:
            if each_char.isalpha():
                alpha_count+=1
        return float(alpha_count)/float(len(text))

    #counting digits (seenu.andi-rajendran)
    def digit_chars_ratio(self, sent_spans_index_start, sent_spans_index_end):
        spans = self.sentence_spans[sent_spans_index_start:sent_spans_index_end]
        start, end = spans[0][0], spans[-1][1]
        text = self.text[start:end]
        digit_count=0
        for each_char in text:
            if each_char.isdigit():
                digit_count+=1
        return float(digit_count)/float(len(text))

    ##counting uppercase characters (seenu.andi-rajendran
    def upper_chars_ratio(self, sent_spans_index_start, sent_spans_index_end):
        spans = self.sentence_spans[sent_spans_index_start:sent_spans_index_end]
        start, end = spans[0][0], spans[-1][1]
        text = self.text[start:end]
        upper_char_count=0
        for each_char in text:
            if each_char.isupper():
                upper_char_count+=1
        return float(upper_char_count)/float(len(text))

    #counting whitespaces (seenu.andi-rajendran)
    def white_chars_ratio(self, sent_spans_index_start, sent_spans_index_end):
        spans = self.sentence_spans[sent_spans_index_start:sent_spans_index_end]
        start, end = spans[0][0], spans[-1][1]
        text = self.text[start:end]
        white_char_count=0
        for each_char in text:
            if each_char.isspace():
                white_char_count+=1
        return float(white_char_count)/float(len(text))



    def sichels_s(self, sent_spans_index_start, sent_spans_index_end):
        return self.hapax_dislegomena(sent_spans_index_start, sent_spans_index_end)/len(self.__wordcount(sent_spans_index_start, sent_spans_index_end))


    def brunets_w(self, sent_spans_index_start, sent_spans_index_end):
        wordcounts=self.__wordcount(sent_spans_index_start, sent_spans_index_end)
        vocab_size = float(len(wordcounts))
        corpus_size = float(sum(wordcounts.values()))
        return corpus_size ** (vocab_size ** -0.172)

    def __calcv(self, sent_spans_index_start, sent_spans_index_end):
        wordcounts = self.__wordcount(sent_spans_index_start, sent_spans_index_end)
    ...
    #adapted from https://github.com/NLPG11/sentiments/blob/master/sFeature.py (seenu.andi-rajendran)
    def simpsons_d(self, sent_spans_index_start, sent_spans_index_end):
        V=self.__calcv(sent_spans_index_start, sent_spans_index_end)
        rs = 0.0
        wordcounts = self.__wordcount(sent_spans_index_start, sent_spans_index_end)
        ...
        for counts in wordcounts.itervalues():
            try:
                rse = V[counts] * (counts / total) * ((counts - 1)/(total - 1))
            except ZeroDivisionError:
                rse =0
            rs += rse
        return rs

    #adapted from https://github.com/shivam5992/textstat/blob/master/textstat/textstat.py (seenu.andi-rajendran)
    def difficult_words(self, sent_spans_index_start, sent_spans_index_end):
        spans = self.sentence_spans[sent_spans_index_start:sent_spans_index_end]
        start, end = spans[0][0], spans[-1][1]
        text = self.text[start:end]
        wds = nltk.tokenize.word_tokenize(text)
        ...
        diff_words = []
        for wd in wds:
            if wd not in easy_word_list:
                if self._wdsyl(wd) > 1:
                    if wd not in diff_words:
                        diff_words.append(wd)
        return len(diff_words)
    #adapted from https://github.com/wimmuskee/readability-score/blob/master/readability_score/calculators/dalechall.py (seenu.andi-rajendran)
    def dale_chall_readability_score(self, sent_spans_index_start, sent_spans_index_end):
        wordcount = float(sum(self.__wordcount(sent_spans_index_start, sent_spans_index_end).itervalues()))
        count = wordcount - self.difficult_words(sent_spans_index_start, sent_spans_index_end)
        if wordcount > 0:
            per = float(count)/float(wordcount)*100
        else:
            print "Can't divide as word count is zero"
            return
        difficult_words = 100-per
        if difficult_words > 5:
            score = (0.1579 * difficult_words) + (0.0496 * self.average_sentence_length(sent_spans_index_start, sent_spans_index_end)) + 3.6365
        else:
            score = (0.1579 * difficult_words) + (0.0496 * self.average_sentence_length(sent_spans_index_start, sent_spans_index_end))
        return round(score, 2)

    #adapted from https://github.com/shivam5992/textstat/blob/master/textstat/textstat.py
    def polysyllablcount(self, sent_spans_index_start, sent_spans_index_end):
        count = 0
        spans = self.sentence_spans[sent_spans_index_start:sent_spans_index_end]
        start, end = spans[0][0], spans[-1][1]
        text = self.text[start:end]
    ...
    def smog_index(self, para_spans_index_start, para_spans_index_end):
        spans = self.paragraph_spans[para_spans_index_start:para_spans_index_end]
        start, end = spans[0][0], spans[-1][1]
        text = self.text[start:end]
        if len(nltk.tokenize.word_tokenize(text)) >= 0:
            try:
                ps = float(self.polysyllablcount(para_spans_index_start, para_spans_index_end))
                smog = (1.043 * (30*(ps/float(len(nltk.sent_tokenize(text)))))**.5) + 3.1291
                return round(smog,1)
            except:
                print "Error: Sentence count is zero"
                return 0
        else:
            return 0

    def average_letter_per_word(self, sent_spans_index_start, sent_spans_index_end):
        spans = self.sentence_spans[sent_spans_index_start:sent_spans_index_end]
        start, end = spans[0][0], spans[-1][1]
        text = self.text[start:end]
        words = text.split()
    ...
    #adapted https://github.com/wimmuskee/readability-score/blob/master/readability_score/calculators/colemanliau.py (seenu.andi-rajendran)
    def coleman_liau_index(self, sent_spans_index_start, sent_spans_index_end):
        S = round(self.average_sentence_length(sent_spans_index_start, sent_spans_index_end)*100, 2)
        L = round(self.average_letter_per_word(sent_spans_index_start, sent_spans_index_end)*100, 2)
        CLI = float((0.058 * L) - (0.296 * S) - 15.8)
        return round(CLI, 2)

    #adapted from https://github.com/wimmuskee/readability-score/blob/master/readability_score/calculators/ari.py (seenu.andi-rajendran)
    def automated_readability_index(self, para_spans_index_start, para_spans_index_end):
        spans = self.paragraph_spans[para_spans_index_start:para_spans_index_end]
        start, end = spans[0][0], spans[-1][1]
        text = self.text[start:end]
        wrds = len(text.split())
        chrs = len(text)
        snts = len(nltk.tokenize.word_tokenize(text))#change here
        try:
    ...
    #adapted from https://github.com/shivam5992/textstat/blob/master/textstat/textstat.py (seenu.andi-rajendran)
    def linsear_write_formula(self, sent_spans_index_start, sent_spans_index_end):
        spans = self.sentence_spans[sent_spans_index_start:sent_spans_index_end]
        start, end = spans[0][0], spans[-1][1]
        text = self.text[start:end]
        easywords = []
        ...
        for i, val in enumerate(textlist):
            if i<= 101:
                try:
                    if self._wdsyl(val) < 3:
                        easywords.append(val)
                    elif self._wdsyl(val) > 3:
                        diffwords.append(val)
                    text = ' '.join(textlist[:100])
                    num = float((len(easywords)*1 + len(diffwords)*3)/len(nltk.sent_tokenize(text)))
                    if num > 20:
                        num /= 2
                    else:
                        num = (num-2)/2
        ...
        for start, end in self.word_spans:
            ...
            try:
                ...
                with open("SyntacticComplexityError.txt", "w") as error_file:
                    error_file.write(str(self.text[self.word_spans[start][0]:]))
                    error_file.write("-------")
                    error_file.write(self.word_spans[start:])
                    error_file.write("-------")
    ...
    def _init_avg_internal_word_freq_class(self):
        ...
        for span in self.word_spans:
            ...
            word = self.text[span[0]:span[1]].lower().translate(dict((x, None) for x in string.punctuation))
