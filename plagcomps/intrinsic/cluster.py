...
from numpy import array, matrix, random, mean, dot
from scipy.cluster.vq import kmeans2, whiten
from scipy.spatial.distance import pdist, cosine
from scipy.cluster.hierarchy import linkage, fcluster
from collections import Counter
from outlier_detection import density_based
...
def cluster(method, k, items, **kwargs):
    ...
    elif method == "cos": #added cos to outlier detection options based on Zechner, Mario, et al. "External and intrinsic plagiarism detection using vector space models." Proc. SEPLN. Vol. 32. 2009. APA	(seenu.andi-rajendran)
        return _cos(items)
...
#cos function definition,works when selecting multiple features (seenu.andi-rajendran)
def _cos(stylo_vectors):
    n = len(stylo_vectors)
    vecs = array(stylo_vectors)
    #print stylo_vectors
    mean_vec = mean(vecs, axis = 0)
    dists = []
    for vec in vecs:
        dists.append(float(cosine(vec, mean_vec)))
    #print dists
    cos_mean = mean(dists)
    sqs=0.00
    for dist in dists:
        sqs += float((dist - cos_mean)**2)
    stddev = math.sqrt(sqs/n)
    confs = [(dist - cos_mean + stddev) for dist in dists]
    confidences = []
    for conf in confs:
        v=(conf-min(confs))/(max(confs)-min(confs))
...
def _combine_confidences(feature_vectors, feature_confidence_weights=None):
    ...
    confidence_vectors = []
    for fi in xrange(len(feature_vectors[0])): # Run as many times as there are vectors
        single_feature_vecs = [[x[fi]] for x in feature_vectors]
        confs = _kmeans(single_feature_vecs, 2)
        for i, confidence in enumerate(confs, 0): #enumerate to show number of each item
